...
parser.add_argument('--log-file', type=str, default='',
                    help='path to save training/validation/test metrics')
...
if args.log_file:
    ...
    log_fh.write("split\tepoch\tbatch\tloss\tppl\n")
...
# accelerator adapted to match my hardware
if args.accel and torch.backends.mps.is_available():
    device = torch.device("mps")
elif args.accel and torch.cuda.is_available():
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            ## for log file 
            if log_fh:
                log_fh.write(f"train\t{epoch}\t{batch}\t{cur_loss:.6f}\t{math.exp(cur_loss):.6f}\n")
                log_fh.flush()
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        # train_loss_epoch = evaluate(train_data)
        # ## saving to log file
        # if log_fh:
        #     log_fh.write(f"train\t{epoch}\t{train_loss_epoch:.6f}\t{math.exp(train_loss_epoch):.6f}\n")
        #     log_fh.flush()
        ...
        ## saving to log file
        if log_fh:
            log_fh.write(f"valid\t{epoch}\tend\t{val_loss:.6f}\t{math.exp(val_loss):.6f}\n")
            log_fh.flush()
...
## for log file
if log_fh:
    log_fh.write(f"test\tfinal\tend\t{test_loss:.6f}\t{math.exp(test_loss):.6f}\n")
    log_fh.flush()
    log_fh.close()
